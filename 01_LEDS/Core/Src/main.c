...
/* USER CODE BEGIN PV */
int clockwise = 1;
uint32_t current_pin = LED12_Pin;
/* USER CODE END PV */
...
int main(void) {
	...
	while (1) {
		if (HAL_GPIO_ReadPin(key_GPIO_Port, key_Pin))
			clockwise = !clockwise;
		HAL_GPIO_WritePin(GPIOD, current_pin, GPIO_PIN_SET);
		HAL_Delay(400);
		HAL_GPIO_WritePin(GPIOD, current_pin, GPIO_PIN_RESET);
		if(clockwise) {
			if(current_pin >= LED15_Pin) current_pin = LED12_Pin;
			else current_pin <<= 1;
		}
		else {
			if(current_pin <= LED12_Pin) current_pin = LED15_Pin;
			else current_pin >>= 1;
		}
